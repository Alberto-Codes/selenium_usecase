...
def test_load_config():
    """
    Test if load_config loads the correct configuration from a JSON file.
...
def test_process_use_cases(mock_driver, config):
    """
    Test if process_use_cases correctly processes and automates the use cases.
    ...
    Raises:
        AssertionError: If the process does not follow the expected workflow.
    """
    with patch("src.selenium_helper.pd.read_excel") as mock_read_excel, patch(
        "src.selenium_helper.tempfile.TemporaryDirectory"
    ) as MockTempDir, patch("src.selenium_helper.load_config", return_value=config):

        mock_read_excel.return_value.iterrows.return_value = iter(
            [
                (
                    0,
                    {
        ...
        MockTempDir.return_value.__enter__.return_value = "/mock/download/dir"

        mock_driver.window_handles = ["main_window", "new_window"]

        process_use_cases("spreadsheet.xlsx", "config.json")
        ...
        mock_driver.find_element.assert_any_call(
            By.CSS_SELECTOR, config["selectors"]["popup_download_button"]
        )
        mock_driver.quit.assert_called_once()
